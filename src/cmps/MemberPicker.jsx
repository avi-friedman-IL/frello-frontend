...
export function MemberPicker({ board, task, onUpdated, setTaskMembers, taskMembers }) {
   const membersBoard = board.members
   const availableMembers = membersBoard.filter(
      boardMember => !taskMembers.some(taskMember => taskMember._id === boardMember._id)
   )
   ...
   function handleAddMember(ev, member) {
      ev.stopPropagation()
      const updateBoardMembers = selectedMembers.filter(m => m._id !== member._id)
      setSelectedMembers(updateBoardMembers)

      const updateTaskMembers = taskMembers?.length ? [...taskMembers, member] : [member]
      onUpdated('members', updateTaskMembers)
      setTaskMembers(updateTaskMembers)
   }
